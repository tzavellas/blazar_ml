...
class Model(object):
    ...
    def __init__(self, model_path, train_mode=True, input_dim=25, T=180, prev=16,
        ...
        self.DO_SHARE=None
        self.x = tf.compat.v1.placeholder(tf.float32, shape=[None, self.input_dim])
        self.ymax = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
        self.y = tf.compat.v1.placeholder(tf.float32, shape=[None, self.T])
        ...
        for t in range(self.T): # range(self.T): if using python3
            ...
            self.e_loss +=tf.reduce_mean(input_tensor=tf.square(y_true - self.ys[t])) #MSE
        ...
        self.eloss_summary = tf.compat.v1.summary.scalar('eloss', self.e_loss)
        self.ploss_summary=tf.compat.v1.summary.scalar('ploss',self.p_loss)
        self.file_writer = tf.compat.v1.summary.FileWriter(logdir, tf.compat.v1.get_default_graph())
...
#                pdb.set_trace()
    ...
    def linear(self, x):
        with tf.compat.v1.variable_scope("e_linear", reuse=self.DO_SHARE):
            yl = tf.contrib.layers.fully_connected(inputs=x, num_outputs=1, activation_fn=None)
        return yl # output logits w.r.t sigmoid
    
    def input_embedding(self, x):
        with tf.compat.v1.variable_scope("e_eblinear1", reuse=None):
            h1 = tf.contrib.layers.fully_connected(inputs=x, num_outputs=128, activation_fn=tf.nn.relu)
        with tf.compat.v1.variable_scope("e_eblinear2", reuse=None):
            h2 = tf.contrib.layers.fully_connected(inputs=x, num_outputs=64, activation_fn=tf.nn.relu)
        return h2
    
    def height_model(self, x):
        with tf.compat.v1.variable_scope("e_hlinear1", reuse=None):
            h1 = tf.contrib.layers.fully_connected(inputs=x, num_outputs=64, activation_fn=tf.nn.relu)
        with tf.compat.v1.variable_scope("e_hlinear2", reuse=None):
            h2 = tf.contrib.layers.fully_connected(inputs=x, num_outputs=1, activation_fn=None)
        return h2
